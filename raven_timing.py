...
with open('./intelData/10Mar1-01.json') as i:
	...
	for packet in a:
		...
		if packet_relTime in timeCounts1:
			timeCounts1[packet_relTime] = timeCounts1[packet_relTime] + 1
		else:
			timeCounts1[packet_relTime] = 1
	...
	fig1 = plt.figure()
	plt.scatter(list(timeCounts1.keys()), timeCounts1.values(), color='g')
	#plt.grid()
